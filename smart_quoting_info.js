(function () {
  ...
  SmartQuotingInfo.prototype._readBinding = function (binding) {
    ...
    // DEBUG: dump every key and its properties into the nodata div
    var dbg = "KEYS=" + keys.join(",") + " || ";
    for (var k = 0; k < keys.length; k++) {
      var v = row[keys[k]];
      if (v && typeof v === "object") {
        dbg += keys[k] + ":{" + Object.keys(v).map(function(p){ return p+"="+String(v[p]); }).join(",") + "} ";
      } else {
        dbg += keys[k] + "=" + String(v) + " ";
      }
    }
    ...
    if (cell && cell.rawValue !== null && cell.rawValue !== undefined) { raw = cell.rawValue; }
    ...
    this._showNoData(true); // force debug visible
  };
  ...
  SmartQuotingInfo.prototype._render = function () {
    ...
    var nd = this._root.getElementById("nodata");
    nd.textContent = this._debugDump ? "DEBUG: " + this._debugDump : this._noDataText;
    nd.style.wordBreak = "break-all";
    nd.style.fontSize = "9px";
  };
