(function () {
  ...
  tmpl.innerHTML =
    ...
    "<svg id='donut-svg' viewBox='0 0 80 80' width='80' height='80'>" +
    "<circle id='arc-track' cx='40' cy='40' r='30' fill='none' stroke='#e0e0e0' stroke-width='10'/>" +
  ...
  WinRateDonut.prototype.connectedCallback = function () {
    ...
    // New properties
    this._trackColor     = "#e0e0e0";
    this._strokeWidth    = 10;
    this._donutSize      = 80;
    this._labelColor     = "#6a6d70";
    this._labelFontSize  = 13;
  ...
  WinRateDonut.prototype.onCustomWidgetAfterUpdate = function (changed) {
    ...
    if ("strokeWidth"    in changed) this._strokeWidth    = changed.strokeWidth;
    if ("donutSize"      in changed) this._donutSize      = changed.donutSize;
    ...
    if ("showLabel"      in changed) this._showLabel      = changed.showLabel;
    if ("borderColor"    in changed) this._borderColor    = changed.borderColor;
    if ("showBorder"     in changed) this._showBorder     = changed.showBorder;
  ...
  WinRateDonut.prototype._render = function () {
    ...
    var track  = this._root.getElementById("arc-track");
    var svg    = this._root.getElementById("donut-svg");
    var card   = this._root.getElementById("card");
    var kpi    = this._root.getElementById("kpi");
    var lbl    = this._root.getElementById("lbl");
    var nodata = this._root.getElementById("nodata");
 
    // --- Donut geometry ---
    svg.setAttribute("width",  this._donutSize);
    svg.setAttribute("height", this._donutSize);
    track.setAttribute("stroke-width", this._strokeWidth);
    arc.setAttribute("stroke-width",   this._strokeWidth);
    track.setAttribute("stroke", this._trackColor);
 
    // --- Arc fill ---
    if (rate === null) {
      arc.setAttribute("stroke-dasharray",  circ.toFixed(2));
      arc.setAttribute("stroke-dashoffset", circ.toFixed(2));
      arc.setAttribute("stroke", this._trackColor);
      kpi.textContent  = "-";
      kpi.style.color  = "#b0b0b0";
    } else {
      var offset = circ * (1 - rate / 100);
      arc.setAttribute("stroke-dasharray",  circ.toFixed(2));
      arc.setAttribute("stroke-dashoffset", offset.toFixed(2));
      arc.setAttribute("stroke", color);
      kpi.textContent = Math.round(rate) + "%";
      kpi.style.color = color;
    }
 
    // --- KPI text ---
    kpi.style.fontSize = this._fontSize + "px";
 
    // --- Label ---
    lbl.textContent   = this._title;
    lbl.style.color   = this._labelColor;
    lbl.style.fontSize = this._labelFontSize + "px";
    lbl.style.display = this._showLabel ? "block" : "none";
    ...
    if (this._showBorder) {
      card.style.border = "1px solid " + this._borderColor;
    } else {
